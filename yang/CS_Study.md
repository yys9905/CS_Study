...
3. [데이터베이스](#데이터베이스)
   1) [트랜잭션](#트랜잭션과-트랜잭션-격리-수준에-대해-설명해주세요-231103)
4. [자료구조/알고리즘](#자료구조/알고리즘)
5. [암호학/보안](#암호학/보안)
6. [프로그래밍 패러다임](#프로그래밍-패러다임)
...
# 데이터베이스스
## 트랜잭션과 트랜잭션 격리 수준에 대해 설명해주세요. (231103)
### 트랜잭션
데이터베이스의 상태를 변환시키는 하나의 논리적 기능을 수행하기 위한 작업의 단위   
(한꺼번에 수행되어야 할 일련의 연산들)
ex) 게시글 게시할떄의 트랜잭션 = (게시 요청 -> DB 업데이트(insert) -> 게시판 업데이트(select) -> 게시 완료)
- 성질
   - 원자성 : 하나의 트랜잭션은 Commmit 되거나 Rollback된다.
   - 일관성 : 처리결과가 항상 일관성이 있어야한다. - 트랜잭션 진행중 데이터변경이 있어도 업데이트된 데이터베이스로 트랜잭션 진행 x
   - 독립성 : 트랜잭션 사이에 간섭 불가능 A트랜잭션이 진행중인 경우 B트랜잭션은 A트랜잭션의 결과를 참조할 수 없음
   - 지속성 : 트랜잭션이 완료된경우 결과는, 무슨일이 있어도 영구적으로 반영되어야 함
- 트랜잭션 연산 
   - Commit : 작업이 완료되고 데이터베이스가 일관된 상태에 있을떄, 연산완료를 트랜잭션 관리자에게 알려줌
   - Rollback : 트랜잭션 처리가 비정상적으로 종료되어 일관성이 망가졌을때, 트랜잭션이 행한 모든 연산을 Undo 하는 연산

- 트랜잭션 상태
   - 활동(Active) : 트랜잭션 실행중
   - 실패(Failed) : 트랜잭션 실행에 오류가 발생하여 중단
   - 철회(Aborted) : 트랜잭션이 비정상적으로 종료되어 Rollback 연산 수행
   - 부분완료(Partially Committed) : 트랜잭션의 연산이 끝났지만 Commit연산이 실행되기 직전
   - 완료(Committed) : 트랜잭션 완료
   
- 트랜잭션 격리 수준 (아래로갈수록 높아짐)
격리 수준 증가할 수록 일관성은 증가하지만 동시성은 감소
   - Read UnCommitted
      - Commit이나 Rollback 여부에 상관없이 다른 트랜잭션에서 값을 읽을 수 있음
      - 사용하지 않는 것을 권장
      - Dirty Read 발생 (트랜잭션이 완료되지 않았는데도 다른 트랜잭션에서 볼수 있게되는 현상   
        A: Update(Non Commit) -> B : 실행 -> B : 내용 조회 -> B : A에서 update된 내용 확인)
   - Read Committed
      - Commit 된 데이터만 읽음
      - RDB에서 대부분 기본적으로 사용됨
      - Non-Repeatable Read 발생 (Reapeatable Read 하나의 트랜잭션 내에서 동일한 Select 쿼리 실행시 항상 같은 결과를 보장 해야함   
      A : update -> B : 실행 -> A : Commit -> B 내용 조회 -> A에서 update 된 내용 확인)
      - Undo 영역에서 값을 가져옴
   - Repeatable Read
      - 트랜잭션이 생성되기 이전의 트랜잭션에서 Commit된 데이터만 읽음
      - MySQL, MariaDB가 기본으로 사용
      - MySQL의 경우 ID를 부여하여 트랜잭션 ID보다 작은 번호에서 변경한 것만 읽음
      - Phantom Read 발생 (한 트랜잭션 안에서 일정 범위의 레코드를 두번 이상 읽었을떄 첫번째 쿼리에 없던 레코드가 두번쨰 쿼리에서 나타나는 현상)
   - Serializable
      - 데이터에 접근할때 항상 Loc을 걸고 데이터 조회
      - Phantom Read 발생 X
      - 성능이 떨어짐
